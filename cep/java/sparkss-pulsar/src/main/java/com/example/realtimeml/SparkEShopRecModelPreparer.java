...
public class SparkEShopRecModelPreparer extends SparkDemoCmdApp {
    ...
    private final static String APP_NAME = "SparkEShopRecModelPreparer";

    // Must be set before initializing the "logger" object.
    static { System.setProperty("log_file_base_name", APP_NAME); }
        ...
        Dataset<EShopRecModelDataSpark> recModelDf = eshopRawDf.withColumn("eventTimeTs", to_timestamp(col("eventTime")))
                .withColumn("metaData",
                        concat(
                                ...
                                concat_ws(
                                        ", ",
                                        concat(lit("\"order\":\""), col("order"), lit("\"")),
        ...
        if (streamingMode) {
            try {
                ...
                streamingQuery = recModelDf
                        .groupBy(
                            col("session"),
                            // A Tumbling window with 10 second size
                            functions.window(col("eventTime"), "10 seconds"))
