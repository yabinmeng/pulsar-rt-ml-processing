...
def create_pulsar_topic_schema(clnt_conn_configs, tnt, ns, topic):
    ...
    if not os.path.isfile(topic_schema_def_file):
        raise Exception("Can't find the corresponding topic schema defintion file (" + topic + ".json)"
                        "under \"../conf/pulsar-schema/\" sub-folder!")
...
def create_cass_table(session, keyspace, table):
    ...
    if not os.path.isfile(tbl_stmt_def_file):
        raise Exception("Can't find the corresponding CQL statement file (" + table + ".stmt)"
                        "under \"../conf/cql-cassandra/\" sub-folder!")
        ...
        while record_line:
            if (record_num == -1) or (record_loaded < record_num):
                ...
                for idx in range(0, len(rawfield_values)):
                    # 'model' and 'so_key' columns are of "text" type
                    if idx == 7 or idx == 15:
        ...
        if b_create_astra_streaming_topic:
            ...
            if 200 <= res.status_code < 300:
                ...
                print("Failed to create the specified Pulsar topic "
                      "(" + pulsar_tenant + "/" + pulsar_namespace + "/" + pulsar_topic_raw + ") with return code: " + str(res.status_code))
